...
public class LoadDataWorker implements Runnable
{
	...
	try
	{
		...
		if (job == 0)
		{
		    ...
		    fmt.format("Worker %03d: Loading ITEM, start time : %s", worker, startDate );
		    ...
		    fmt.format("Worker %03d: Loading ITEM done, end time: %s, running time %d seconds ", worker,endDate, runTimeMS/1000 );
		    System.out.println(sb.toString());
		    sb.setLength(0);
		}
		else
		{
		    ...
		    fmt.format("Worker %03d: Loading Warehouse %6d, start time : %s",
			       worker, job, startDate);
		    System.out.println(sb.toString());
		    sb.setLength(0);
		    ...
		    fmt.format("Worker %03d: Loading Warehouse %6d done, end time: %s, running time %d seconds ",
			       worker, job, endDate, runTimeMS/1000 );
		    System.out.println(sb.toString());
		    sb.setLength(0);
		}
	    }
	...
	for (int s_i_id = 1; s_i_id <= 100000; s_i_id++)
	{
	    String sData;
	    /*
	     * Load the data in batches of 1,000 rows.
	     */
       ...
       fmt.format("Worker %03d: Loading Warehous %d. Stock is %6d done ",
                worker, w_id, record );
       System.out.println(sb.toString());
       sb.setLength(0);
	...
	for (int d_id = 1; d_id <= 10; d_id++)
	{
		...
		stmtHistory.clearBatch();
            ...
            //fmt.format("Worker %03d: Loading Warehouse %d, District %d Customer and History, each Records: 3000, done ",
            //    worker, w_id, d_id );
       	    //System.out.println(sb.toString());
            //sb.setLength(0);
	    ...
	    fmt.format("Worker %03d: Loading Warehouse %d,  District is %d, done ",
			    worker, w_id,  d_id );
	    System.out.println(sb.toString());
	    sb.setLength(0);
